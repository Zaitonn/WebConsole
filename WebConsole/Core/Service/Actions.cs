...
namespace Serein.Plugins.WebConsole.Core.Service
{
    internal static class Actions
    {
        ...
        public static void Handle(Console console, ReceivedPacket packet)
        {
            ...
            switch (packet.SubType)
            {
                ...
                case "server_stop":
                case "server_kill":
                    if (!CheckTarget(console, packet, out subAll, out instance))
                    {
                        break;
                    }
        ...
        private static void Send(Console console, string subType, object data, bool subAll = false, Instance instance = null)
        {
            if (subAll)
            {
                ...
                {
                    Connections.Instances.Values.ToList().ForEach((enumeredInstance) => enumeredInstance?.Send(new SentPacket("action", subType, data, console.AsSender()).ToString()));
                }
            }
            else
            {
                instance?.Send(new SentPacket("action", subType, data, console.AsSender()).ToString()).Await();
            }
        }
    }
}